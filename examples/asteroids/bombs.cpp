...
void Bombs::update(Ship &ship, const GameData &gameData, float deltaTime, AmmoBar &ammoBar) { // 
  ...
  if (gameData.m_input[static_cast<size_t>(Input::Bomb)] &&
    ...
    // At least 250 ms must have passed since the last bomb
  ...
  for (auto &bombs : m_bombs) {
    bombs.m_translation -= ship.m_velocity * deltaTime;
    bombs.m_translation += bombs.m_velocity * deltaTime;

    // Kill bombs if it goes off screen
    if (bombs.m_translation.x < -1.1f) bombs.m_dead = true;
    if (bombs.m_translation.x > +1.1f) bombs.m_dead = true;
    if (bombs.m_translation.y < -1.1f) bombs.m_dead = true;
    if (bombs.m_translation.y > +1.1f) bombs.m_dead = true;
  }
